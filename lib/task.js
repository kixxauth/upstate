...
exports.newTask = Objects.factory([EventEmitter], {
  ...
  initialize: function (spec) {
    var
    options = parseOptions(spec.id, spec.usage, spec.options);
...
function parseOptions(id, usage, options) {
  ...
  Object.keys(options).forEach(function (key) {
    var
    conf = options[key] || Object.create(null);

    if (conf.alias) {
      argv = argv.alias(key, conf.alias);
    }
    if (conf.description) {
      argv = argv.describe(conf.description);
    }
    if (conf.boolean) {
      argv = argv.boolean(conf.boolean);
    }
    if (conf.required) {
